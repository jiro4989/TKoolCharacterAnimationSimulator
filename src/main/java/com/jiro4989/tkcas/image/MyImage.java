...
/**
 * ラッパーImageクラス。このクラスは不変クラスである。 自身のインスタンスを返すメソッドの戻り値はすべて新しく生成されたインスタンス
 * であり、メソッド呼び出しの副作用で元のインスタンスのピクセルデータが変更され ることはない。
 */
public class MyImage {
  ...
  public static class Builder { // {{{
    ...
    public MyImage build() { // {{{
      ...
      if (w < x) throw new IllegalArgumentException("座標xは画像幅widthの値以下でなければいけません。" + this);
  ...
  private MyImage(Builder builder) { // {{{
    ...
    if (imgWidth != bldWidth || imgHeight != bldHeight) {
    ...
    range(0, row)
        .forEach(
            r -> {
              ...
              CharaChipGridPane ccgp =
                  new CharaChipGridPane.Builder(this, width, height, pos)
                      .x(x)
                      .y(y)
